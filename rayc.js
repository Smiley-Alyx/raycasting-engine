...
var renderer = createRenderer({ ctx, getViewWidth, getViewHeight, player });
...
function render(){ //Рендеринг состоит из трёх шагов:
  renderer.drawBackground(); //Рисуем пол и потолок
  castRays({ player, getViewWidth, addRotToAngle, drawRay: renderer.drawRay }); //Бросаем лучи и отрисовываем стены
  if(player.flatmap) renderer.drawMap(); //Отрисовываем плоскую карту 
}
