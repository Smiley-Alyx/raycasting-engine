...
function hitWall(x, y) { //проверка на попадание в стену(как в js перегружать функции???????????)
  // Если вышли за границы карты — считаем, что там стена.
  // Это защищает от undefined и делает поведение предсказуемым.
...
function castSingleRay(angle, row) { //бросание лучей по одному
  ...
  var img = 0; //номер текстуры/материал
  ...
  while (x >= 0 && x < map[0].length && y >= 0 && y < map.length)//Двигаемся по карте уровня
  {
    ...
    if (hitWall(xMap, yMap)){ //DDA в здании!
      distH = Math.abs((player.x - x) / Math.cos(angle)); //Спасайте дистанции
      xHitH = x;
      yHitH = y;
      offsetH = x % 1; //Слушайте дядю Хованского
      imgH = cellLegend[String(map[yMap][xMap])] || map[yMap][xMap]; //Я тебе зуб даю, бля буду гангстером! 
      break;
    }
  ...
  while (x >= 0 && x < map[0].length && y >= 0 && y < map.length)
  {
    ...
    if (hitWall(xMap, yMap)){
      ...
      yHitV = y;
      offsetV = y %1;
      imgV = cellLegend[String(map[yMap][xMap])] || map[yMap][xMap];
      break;
    }
    x += dX;
    y += dY;
  }
  if (distV < distH){ //если дистанция вертикального меньше горизонтального - то есть вертикальный луч ближе
    x = xHitV;
    y = yHitV;
    dist = distV;
    img = imgV;
